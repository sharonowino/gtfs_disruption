...
try:
    ...
    logger.warning("PyTorch not available - GNN models disabled")
...
class DisruptionPipeline:
    ...
    def __init__(self, config_path: str = None):
        ...
        if self.config is None:
            logger.warning("Config load failed, using default config")
            self.config = {}
        try:
            setup_logging(self.config)
        except Exception as e:
            logger.warning(f"Logging setup failed: {e}, using default logging")
    ...
    def _get_edge_index(self, G: nx.DiGraph = None) -> Optional[np.ndarray]:
        ...
        try:
            ...
            logger.warning(f"PyTorch error in _get_edge_index: {e}")
            TORCH_AVAILABLE = False
            return None
    ...
    NETHERLANDS_LAT_MIN = 50.75
    NETHERLANDS_LAT_MAX = 53.55
    NETHERLANDS_LON_MIN = 3.30
    NETHERLANDS_LON_MAX = 7.20
    ...
    def run_full_pipeline(
        ...
        if use_adaptive_split:
            try:
                ...
                logger.warning(f"  Adaptive split failed: {e}. Skipping split info.")
                split_info = None
        
        # Run rolling window simulation
        if model_type == "all":
            # Use fixed split for small datasets (forwards compatibility check)
            # First try rolling window, fall back if insufficient data
            try:
                ...
                logger.warning(f"  Rolling window failed: {e}. "
...
if __name__ == "__main__":
    ...
    merged_df, gtfs_data = pipeline.ingest_data()
    result = pipeline.run_full_pipeline(merged_df, gtfs_data)
